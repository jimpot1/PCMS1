...
| P3 | **Supplies Inventory** | Department-based supplies; stock-in/write-off movement ledger; minimum/reorder level; low-stock alert; automatic requisition setting. | Create supply/opening balance → record stock-in or authorized write-off → update on-hand quantity → compare with reorder level → alert/requisition if low → restock and resolve condition. Approved request releases remain system-generated issuance movements. |
